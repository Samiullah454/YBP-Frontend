import { Component, OnInit, ViewChild, ViewEncapsulation } from '@angular/core';
import { NgbModal } from '@ng-bootstrap/ng-bootstrap';
import { ColumnMode, DatatableComponent } from '@swimlane/ngx-datatable';
import Swal from 'sweetalert2';

import { ServiceContractService } from '../service-contract.service';
import { ServiceContractaddComponent } from '../service-contractadd/service-contractadd.component';

@Component({
  selector: 'app-service-contractlist',
  templateUrl: './service-contractlist.component.html',
  styleUrls: ['./service-contractlist.component.scss'],
  encapsulation: ViewEncapsulation.None
})
export class ServiceContractlistComponent implements OnInit {
  public selectedOption = 10;
  public searchValue = '';
  public rows: any[]=[];
  Types: any[] = [];
  public ColumnMode = ColumnMode;
  TypeMap: { [key: number]: string } = {};

  constructor(private _serviceContract:ServiceContractService,private modalService: NgbModal) { }
  @ViewChild(DatatableComponent) table: DatatableComponent;

  ngOnInit(): void {
    this.fetchServiceContractTypes();
    this.fetchServiceContract();
  
  }
 
 
  
  // Method to get the shift name based on the number

  edit(row: any) {

    this.openAddServiceContractModal(row);
  }
  openAddServiceContractModal(Shift?: any) {
    const modalRef = this.modalService.open(ServiceContractaddComponent);
    modalRef.componentInstance.serviceToEdit = Shift; 

    modalRef.result.then((result) => {
      this.fetchServiceContract();
    }, (reason) => {
    });
  }
  fetchServiceContractTypes(): void {
    
    this._serviceContract.getAllServiceContractsTypes().subscribe(
      (data: any) => {
        this.Types = data.result.items
   console.log(this.Types)
      },
      (error: any) => {
        console.error('Error fetching Types:', error);
      }
    );
  }
  fetchServiceContract(): void {
    
    this._serviceContract.getAllServiceContracts().subscribe(
      (data: any) => {
        this.rows = data.result.items
        
        console.log(this.Types)

        
        console.log(this.rows)
      },
      (error: any) => {
        console.error('Error fetching industries:', error);
      }
    );
  }
  getTypeName(number: number): string {
    const foundType = this.Types.find(type => type.id === number);

    return foundType ? foundType.name : 'Unknown';
  }
  ConfirmTextOpen(id) {
    Swal.fire({
      title: 'Are you sure?',
      text: "You won't be able to revert this!",
      icon: 'warning',
      showCancelButton: true,
      confirmButtonColor: '#7367F0',
      cancelButtonColor: '#E42728',
      confirmButtonText: 'Yes, delete it!',
      customClass: {
        confirmButton: 'btn btn-primary',
        cancelButton: 'btn btn-danger ml-1'
      }
    }).then((result) => {
      if (result.value) {
        this.deleteService(id);
      }
    });
  }
  
  deleteService(id: any) {
    // Implement your logic to delete the industry
    // For example, call the delete method from your service passing the industry ID
    this._serviceContract.DeleteServiceContract(id).subscribe(
      (response: any) => {
        // Handle success response
        console.log('Industry deleted:', response);
        // Optionally, refresh the industry list after deletion
        this.fetchServiceContract();
      },
      (error: any) => {
        // Handle error response
        console.error('Error deleting Service Contract:', error);
      }
    );
  }
}
